...
function dvwaHtmlEcho( $pPage ) {
	...
	if( dvwaIsLoggedIn() ) {
		$menuBlocks[ 'meta' ][] = array( 'id' => 'security', 'name' => 'WebSec Security', 'url' => 'security.php' );
	...
	$menuHtml = '';
	foreach( $menuBlocks as $section => $menuBlock ) {
	...
	$menuHtml .= '<ul id="toggleVulnerabilities"><b>Show Vulnerabilities</b></ul>';
	$menuHtml .= '<div id="vulnerabilitiesMenu" style="display:none;"><ul class="menuBlocks">';
	foreach( $menuBlocks['vulnerabilities'] as $menuItem ) {
        $selectedClass = ( $menuItem[ 'id' ] == $pPage[ 'page_id' ] ) ? 'selected' : '';
        $fixedUrl = DVWA_WEB_PAGE_TO_ROOT.$menuItem[ 'url' ];
...
foreach( $menuBlocks as $section => $menuBlock ) {
	...
	$menuHtml .= <<<EOD
	<script>
	document.getElementById('toggleVulnerabilities').addEventListener('click', function() {
		var menu = document.getElementById('vulnerabilitiesMenu');
		if (menu.style.display === 'none') {
			menu.style.display = 'block';
			this.textContent = 'Hide Vulnerabilities'; // Change button text
			this.classList.add('selected'); // Add the selected style
		} else {
			menu.style.display = 'none';
			this.textContent = 'Show Vulnerabilities'; // Change back the button text
...
<html lang=\"en-GB\">
	...
	<head>
		...
		<link rel=\"stylesheet\" type=\"text/css\" href=\"" . DVWA_WEB_PAGE_TO_ROOT . "dvwa/css/main.css?v=1.4\" />
	...
	<body class=\"home\">
		<div id=\"container\">
			...
			<h1 " . DVWA_WEB_PAGE_TO_ROOT . ">WebSec: Exploring and Modulating Vulnerability</h1>;
			...
			<div id=\"footer\">
				...
				<p>WebSec: Exploring and Modulating Vulnerability</p>
